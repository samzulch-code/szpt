...
function useCalcs(logs: DailyLog[], plan: Plan | null) {
  ...
  return {
    ...
    allDays, actualData, projData, cpr, cprGood, last56, rollingData, actualRolling,
    calTag, hiTag,
  }
}
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 2fr 1fr',gap:'16px' }}>
        <Panel>
          ...
          <div style={{ marginTop:'10px',fontSize:'9px',color:c.cprGood?'#4ade80':'#fca5a5',lineHeight:1.6 }}>
            {c.cpr?(c.cprGood?'✓ Good protein density':`✗ ${(parseFloat(c.cpr)-10).toFixed(1)} above target`):'Log calories and protein'}
          </div>
        </Panel>
        ...
        <Panel>
          ...
          <div style={{ height:'160px' }}>
            <Line data={{ labels:c.last56.map((d,i)=>i%7===0?formatDate(d):''), datasets:[
              {label:'Daily',data:c.actualRolling,borderColor:'rgba(249,115,22,.3)',backgroundColor:'transparent',borderWidth:1,pointRadius:1,tension:.2,spanGaps:true},
              {label:'7-Day Avg',data:c.rollingData,borderColor:'#f97316',backgroundColor:'rgba(249,115,22,.06)',borderWidth:2,pointRadius:0,tension:.4,spanGaps:true,fill:true},
            ]}} options={chartOpts} />
          </div>
        </Panel>
